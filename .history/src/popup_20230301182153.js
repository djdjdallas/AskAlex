...
function Popup() {
  ...
  const MAX_API_CALLS_PER_USER = 3; // set maximum number of API calls per user
  const userId = "user1"; // set user ID
  ...
  async function generateSuggestions() {
    ...
    setIsLoading(true);
    const apiCalls = parseInt(localStorage.getItem(userId)) || 0;
    if (apiCalls >= MAX_API_CALLS_PER_USER) {
      alert(
        `You have reached the maximum number of API calls (${MAX_API_CALLS_PER_USER})`
      );
      setIsLoading(false);
      return;
    }
    ...
    localStorage.setItem(userId, apiCalls + 1);
  }
